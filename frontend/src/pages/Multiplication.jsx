...
export default function Multiplication() {
  ...
  for (let i = 1; i <= TOTAL_LEVELS; i++) {
    const stars = Number(localStorage.getItem(`multiplication_level_${i}_stars`)) || 0;
  ...
  const calculateTotalStars = () => {
    let sum = 0;
    for (let i = 1; i <= TOTAL_LEVELS; i++) {
      sum += Number(localStorage.getItem(`multiplication_level_${i}_stars`)) || 0;
    }
    setTotalStars(sum);
  };
            ...
            return (
              <div
                ...
                className={`absolute w-14 h-14 sm:w-16 sm:h-16 rounded-full flex flex-col items-center justify-center text-white font-bold shadow-lg
                  ${isUnlocked ? "bg-blue-500 cursor-pointer hover:scale-110 transition-transform" : "bg-gray-400"}`}
                style={{
                  left: `${pos.x}%`,
                  top: `${pos.y}%`,
