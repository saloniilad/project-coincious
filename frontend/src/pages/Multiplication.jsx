...
export default function Multiplication() {
  ...
  const recalcUnlocked = () => {
    ...
    while (true) {
      const stars =
        Number(localStorage.getItem(`multiplication_level_${i}_stars`)) || 0;
      if (stars > 0) {
        unlocked = i + 1;
        i++;
      } else break;
  ...
  const handleLevelComplete = (lvl) => {
    ...
    const currentUnlocked =
      Number(localStorage.getItem("multiplication_unlocked")) || 1;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs md:text-sm text-gray-500">
                  Practice multiplication with fun levels
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex justify-center pb-20 px-4">
        <div className="relative w-full h-[90vh]">
          ...
          {Array.from({ length: LEVELS_PER_SET }, (_, i) => {
            ...
            const stars =
              Number(
                localStorage.getItem(`multiplication_level_${level}_stars`),
              ) || 0;
            ...
            return (
              <div
                ...
                className={`absolute w-14 h-14 sm:w-16 sm:h-16 rounded-full flex flex-col items-center justify-center text-white font-bold shadow-lg
                  ${isUnlocked ? "bg-green-500 cursor-pointer hover:scale-110 transition-transform" : "bg-gray-400 cursor-not-allowed opacity-60"}`}
                style={{
                  left: `${pos.x}%`,
                  top: `${pos.y}%`,
