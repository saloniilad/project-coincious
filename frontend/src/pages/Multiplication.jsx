...
export default function Multiplication() {
  ...
  const recalcUnlocked = () => {
    const highest  = getHighestCompletedLevel("multiplication");
  ...
  const recalcTotalStars = () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith("multiplication_level_") && key.endsWith("_stars")) {
  ...
  const handleLevelComplete = (lvl) => {
    recalcTotalStars();
    const currentUnlocked = Number(localStorage.getItem("multiplication_unlocked")) || 1;
    const next = Math.max(currentUnlocked, lvl + 1);
    localStorage.setItem("multiplication_unlocked", next);
    ...
    <div className="min-h-screen bg-[#f3f1ee]">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between px-4 md:px-8 py-4 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl md:text-3xl font-bold text-gray-800">Multiplication</h1>
                <p className="text-xs md:text-sm text-gray-500">Practice multiplication with fun levels</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-start md:items-end gap-2">
            ...
            {totalStars >= STARS_REQUIRED_TO_UNLOCK && (
              <span className="text-green-600 text-xs md:text-sm font-semibold">✅ Division unlocked</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4 py-3 bg-white border-b shadow-sm">
        <button
          ...
          className="px-3 py-1 rounded-lg bg-green-100 text-green-700 font-bold disabled:opacity-30 hover:bg-green-200 transition flex items-center"
        >
          <ChevronLeft size={18} />
        </button>
      ...
      <div className="flex justify-center pb-20 px-4">
        <div className="relative w-full h-[90vh]">
          ...
          {Array.from({ length: LEVELS_PER_SET }, (_, i) => {
            ...
            const stars      = Number(localStorage.getItem(`multiplication_level_${level}_stars`)) || 0;
