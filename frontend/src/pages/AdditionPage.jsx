...
export default function Addition() {
  ...
  const calculateTotalStars = () => {
    ...
    for (let i = 1; i <= TOTAL_LEVELS; i++) {
      sum +=
        Number(
          localStorage.getItem(`addition_level_${i}_stars`)
        ) || 0;
    }
    setTotalStars(sum);
  };

  // 🌸 Manual road positioning (tuned to your image)
    ...
    <div className="min-h-screen bg-[#f3f1ee]">
      ...
      <div className="flex justify-center pb-20 px-4">
        <div className="relative w-full h-[90vh]">
          ...
          {Array.from({ length: TOTAL_LEVELS }, (_, i) => {
            ...
            return (
              <div
                key={level}
                onClick={() =>
                  isUnlocked && setSelectedLevel(level)
                }
          ...
          {unlockedLevel && (
            <div
              className="absolute text-4xl sm:text-5xl transition-all duration-1000"
              style={{
