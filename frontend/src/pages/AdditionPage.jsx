...
export default function Addition() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      loadProgressFromBackend(storedUser).then(() => {
        ...
        recalcTotalStars();
        // Jump to the set that has the current unlocked level
  ...
  const handleLevelComplete = (lvl) => {
    ...
    // Auto-advance to next set if needed
    ...
    <div className="min-h-screen bg-[#f3f1ee]">
      ...
      <div className="flex items-center justify-center gap-4 py-3 bg-white border-b shadow-sm">
        <button
          ...
          className="px-3 py-1 rounded-lg bg-purple-100 text-purple-700 font-bold disabled:opacity-30 hover:bg-purple-200 transition flex items-center"
        >
          <ChevronLeft size={18} />
        </button>
      ...
      <div className="flex justify-center pb-20 px-4">
        <div className="relative w-full h-[90vh]">
            ...
            return (
              <div
                ...
                className={`absolute w-14 h-14 sm:w-16 sm:h-16 rounded-full flex flex-col items-center justify-center text-white font-bold shadow-lg
                  ${
                    isUnlocked
                      ? "bg-purple-500 cursor-pointer hover:scale-110 transition-transform"
          ...
          {/* Plane indicator — only shown if the unlocked level is in this set */}
