...
export default function Subtraction() {
  ...
  const recalcUnlocked = () => {
    const highest  = getHighestCompletedLevel("subtraction");
  ...
  const recalcTotalStars = () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith("subtraction_level_") && key.endsWith("_stars")) {
  ...
  const handleLevelComplete = (lvl) => {
    recalcTotalStars();
    const currentUnlocked = Number(localStorage.getItem("subtraction_unlocked")) || 1;
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between px-4 md:px-8 py-4 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl md:text-3xl font-bold text-gray-800">Subtraction</h1>
                <p className="text-xs md:text-sm text-gray-500">Practice subtraction with fun levels</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-start md:items-end gap-2">
            ...
            {totalStars >= STARS_REQUIRED_TO_UNLOCK && (
              <span className="text-green-600 text-xs md:text-sm font-semibold">✅ Multiplication unlocked</span>
            )}
          </div>
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`absolute w-14 h-14 sm:w-16 sm:h-16 rounded-full flex flex-col items-center justify-center text-white font-bold shadow-lg ${
                  isUnlocked
                    ? "bg-red-400 cursor-pointer hover:scale-110 transition-transform"
