        ...
        {images.map((currency, index) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="text-sm text-amber-700 mt-2 font-semibold">₹{currency.value}</span>
              <span className="text-xs text-gray-400 capitalize">{currency.type}</span>
              <span className="text-[10px] text-pink-400 mt-1">🔍 tap to zoom</span>
            </div>
          );
        })}
      </div>
      ...
      {zoomedIdx !== null && (() => {
        ...
        return (
          <div
            ...
            style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, cursor: "zoom-out" }}
          >
            <div
              onClick={(e) => e.stopPropagation()}
              style={{ background: "white", borderRadius: 24, padding: "32px 40px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", animation: "zoomIn 0.2s ease-out" }}
            >
              <style>{`@keyframes zoomIn { from{transform:scale(0.7);opacity:0} to{transform:scale(1);opacity:1} }`}</style>
              <img src={imgUrl} alt={`₹${c.value}`} style={{ width: 220, height: 220, objectFit: "contain" }} />
              <p style={{ fontSize: 22, fontWeight: 800, color: "#b45309" }}>₹{c.value}</p>
              <p style={{ fontSize: 13, color: "#9ca3af", textTransform: "capitalize" }}>{c.type}</p>
              <button
                onClick={() => setZoomedIdx(null)}
                style={{ marginTop: 4, background: "linear-gradient(135deg,#ec4899,#f472b6)", color: "white", border: "none", borderRadius: 12, padding: "8px 28px", fontWeight: 800, fontSize: 14, cursor: "pointer" }}
              >
                Close ✕
              </button>
            </div>
...
function VisualQuestion({ module, questionText, currencyImages }) {
  ...
  const CoinImg = (
    <div className="flex flex-col items-center cursor-zoom-in group" onClick={() => setZoomed(true)}>
      <div className="relative">
        <img
          ...
          onError={(e) => { e.target.style.display = "none"; }}
        />
        <span className="absolute -top-2 -right-2 bg-white/80 text-pink-500 text-[9px] font-black px-1.5 py-0.5 rounded-full shadow border border-pink-200 backdrop-blur-sm">🔍</span>
      </div>
      <span className="text-xs text-pink-500 font-bold mt-1">₹{coin.value}</span>
    </div>
  );
  ...
  return (
    <>
      <div className="flex items-center justify-center gap-4 flex-wrap mt-2 mb-1">
        {parts.left ? <span className="text-4xl font-black text-[#3b2f1e]">{parts.left}</span> : CoinImg}
        <span className="text-3xl font-black text-pink-400">{parts.op}</span>
        {parts.right ? <span className="text-4xl font-black text-[#3b2f1e]">{parts.right}</span> : CoinImg}
        <span className="text-3xl font-black text-pink-400">=</span>
        <span className="text-4xl font-black text-[#3b2f1e]">?</span>
      </div>

      {zoomed && (
        <div onClick={() => setZoomed(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, cursor: "zoom-out" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: 24, padding: "32px 40px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", animation: "zoomIn 0.2s ease-out" }}>
            <style>{`@keyframes zoomIn{from{transform:scale(0.7);opacity:0}to{transform:scale(1);opacity:1}}`}</style>
            <img src={imgUrl} alt={`₹${coin.value}`} style={{ width: 220, height: 220, objectFit: "contain" }} />
            <p style={{ fontSize: 22, fontWeight: 800, color: "#b45309" }}>₹{coin.value}</p>
            <p style={{ fontSize: 13, color: "#9ca3af", textTransform: "capitalize" }}>{coin.type}</p>
            <button onClick={() => setZoomed(false)} style={{ marginTop: 4, background: "linear-gradient(135deg,#ec4899,#f472b6)", color: "white", border: "none", borderRadius: 12, padding: "8px 28px", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>Close ✕</button>
          </div>
        </div>
      )}
    </>
...
export default function MathGame({ module, symbol, level, onBack, onComplete }) {
  ...
  const [difficulty, setDifficulty]         = useState(() => {
    // CRITICAL: First 3 levels ALWAYS use easy-basic
    // This ensures confidence-building before introducing harder content
  ...
  const fetchCurrencyImages = useCallback(async (currencyIds) => {
    if (!currencyIds || currencyIds.length === 0) { setCurrencyImages([]); return; }
    try {
      const uniqueIds = [...new Set(currencyIds)];
      const res  = await fetch(`${API_BASE}/math/currencies/?ids=${uniqueIds.join(",")}`);
      const data = await res.json();
      const currencyMap = {};
  ...
  const fetchFreshQuestion = useCallback(async (diffOverride) => {
    const actualDifficulty = diffOverride || "easy-basic";
    const res = await fetch(
      `${API_BASE}/math/question/?module=${module}&difficulty=${actualDifficulty}`
    );
    if (!res.ok) throw new Error(`No question found for ${module}/${actualDifficulty}`);
    const data = await res.json();
  ...
  const loadQuestion = useCallback(async () => {
    ...
    try {
      ...
      if (!revisitRes.ok) throw new Error(`Level-question lookup failed: ${revisitRes.status}`);
      ...
      if (revisitData.question_id) {
        ...
        if (byIdRes.ok) {
          ...
          // by-id fetch failed — fall back to a fresh question using stored next_difficulty
          setIsRevisit(false);
          const storedNext = localStorage.getItem(`${module}_next_difficulty`);
          const fallbackDiff = level <= 3
            ? "easy-basic"
        ...
        // Read stored AI difficulty BEFORE removing it
        const storedNext = localStorage.getItem(`${module}_next_difficulty`);
        const startDiff  = level <= 3 ? "easy-basic" : storedNext || "easy-basic";
        // Only clear after we've captured the value
        localStorage.removeItem(`${module}_next_difficulty`);
  ...
  const handleSubmit = async (directAnswer = null) => {
    ...
    const correctAnswer =
      question.problem_type === "mcq" ? question.correct_answer : question.expected_answer;
    ...
    history = history.slice(-10); // keep last 10, AI will use last 3
    localStorage.setItem(historyKey, JSON.stringify(history));

    // Pass the FULL history; adaptiveRNN.js will use the last 3
    ...
    try {
      const res  = await fetch(`${API_BASE}/math/attempt/save/`, {
        ...
        body:    JSON.stringify({
          ...
          difficulty,
          next_difficulty: aiDifficulty,  // stored in DB for analytics
        }),
      });
      const data = await res.json();
      setDeltaStars(data.delta_stars ?? 0);
      setPrevBestStars(data.previous_best_stars ?? 0);
      // ← setDifficulty(aiDifficulty) intentionally removed
    } catch (e) {
    ...
    return (
      <div className="min-h-screen bg-[#f3f1ee] flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl p-8 shadow-lg text-center space-y-4 max-w-sm">
          ...
          <button onClick={loadQuestion} className="bg-pink-500 text-white px-6 py-2 rounded-xl hover:bg-pink-600 transition">
            Retry
          </button>
    ...
    return (
      <div className="min-h-screen bg-[#f3f1ee] flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl p-8 shadow-xl max-w-sm w-full text-center space-y-5">
          <div className="text-5xl">{starsEarned === 3 ? "🎉" : starsEarned === 2 ? "👍" : "💪"}</div>
          <h2 className="text-2xl font-bold text-[#3b2f1e]">Level {level} Complete!</h2>
          <div className="flex justify-center gap-2 text-4xl">
            {[1, 2, 3].map((s) => (
              <span key={s} className={`transition-transform duration-300 ${s <= starsEarned ? "text-yellow-400 scale-125" : "text-gray-300"}`}>★</span>
            ))}
          </div>
          <p className="text-gray-600 font-medium">
            {starsEarned === 3 ? "Perfect! Excellent work! 🌟" : starsEarned === 2 ? "Great job! Keep it up!" : "Good effort! Practice makes perfect!"}
          </p>
          ...
          {deltaStars > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2 text-green-700 text-sm font-medium">
              ...
              {prevBestStars > 0 && <span className="block text-xs text-green-600">(Previous best: {prevBestStars} ★ → Now: {starsEarned} ★)</span>}
            </div>
          )}
  ...
  const isVisualQuestion =
    (module === "multiplication" && currencyImages.length > 0 && question?.question_text?.match(/^(\d+)\s*[×x\*]/)) ||
    (module === "division"       && currencyImages.length > 0 && question?.question_text?.match(/[÷\/]\s*(\d+)/));
  ...
  return (
    <div className="min-h-screen bg-[#f3f1ee] flex flex-col">
      ...
      <div className="max-w-3xl mx-auto w-full px-4 pt-6 flex items-center justify-between">
        <button onClick={onBack} className="bg-blue-200 text-blue-800 px-4 py-2 rounded-2xl flex items-center gap-2 hover:bg-blue-300 transition text-sm">
          <ChevronLeft size={16} /> Back
        </button>
        <div className="flex items-center gap-3">
          <span className="bg-white text-gray-500 border text-xs px-3 py-1 rounded-full shadow-sm">{difficulty}</span>
          <span className="bg-pink-500 text-white text-sm font-bold px-4 py-1 rounded-full shadow">Level {level}</span>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl w-full max-w-xl p-8 space-y-6">
          ...
          <div className="text-center">
            <div className="flex justify-center mb-3">
              <div style={{ background: "linear-gradient(135deg,#ec4899,#f472b6)", borderRadius: "50%", width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 18px rgba(236,72,153,0.45),0 1px 4px rgba(0,0,0,0.1)", fontSize: 22, fontWeight: 900, color: "white", letterSpacing: "-1px", border: "3px solid rgba(255,255,255,0.35)" }}>
                {symbol}
              </div>
            </div>

            {isVisualQuestion ? (
              <VisualQuestion module={module} questionText={question?.question_text} currencyImages={currencyImages} />
            ) : (
              <h2 className="text-2xl font-bold text-[#3b2f1e] leading-snug">{question?.question_text}</h2>
            )}

            {isRevisit && <span className="text-xs text-gray-400 mt-1 block">(Revisiting this level)</span>}

            <div className="flex justify-center mt-4">
              <button
                onClick={() => isSpeaking ? stopSpeaking() : speakQuestion(question?.question_text)}
                ...
                {isSpeaking ? (
                  <>
                    ...
                    <span className="flex items-center gap-1">
                      Reading
                      <span style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 18, marginLeft: 4 }}>
                        {[0, 150, 300].map((delay) => (
                          <span key={delay} style={{ width: 4, borderRadius: 2, background: "white", animation: `voiceBar 0.7s ease-in-out ${delay}ms infinite alternate` }} />
                        ))}
                      </span>
                    </span>
                  </>
          ...
          {!isMCQ && (
            <div className="space-y-2">
              <label className="text-sm text-gray-500 font-medium">Your Answer (₹)</label>
              <input
                type="number"
                value={wordAnswer}
          ...
          {showHintPanel && hintMessages.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 space-y-2">
              {hintMessages.map((msg, i) => <p key={i} className="text-yellow-800 text-sm">💡 {msg}</p>)}
            </div>
          )}

          {attempts > 0 && <p className="text-center text-xs text-gray-400">Attempts so far: {attempts}</p>}
